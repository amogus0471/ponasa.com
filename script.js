...
const languageNames = { en: "English", es: "Spanish", it: "Italian" };
const languageCurrent = document.querySelector("[data-language-current]");
const languageOptions = document.querySelectorAll("[data-language-option]");
const storedLanguage = window.localStorage?.getItem("ponasa-language") ?? "en";
const setLanguage = (language) => {
  const selected = languageNames[language] ? language : "en";
  if (languageCurrent) languageCurrent.textContent = languageNames[selected];
  languageOptions.forEach((option) => option.setAttribute("aria-checked", String(option.dataset.languageOption === selected)));
  window.localStorage?.setItem("ponasa-language", selected);
};
