...
const sendMarketData = async (res, symbols) => {
  ...
  for (const symbol of symbols.slice(0, 5)) {
    try {
      ...
      data[symbol] = { price, change: previous ? ((price - previous) / previous) * 100 : Number(quote["10. change percent"]?.replace("%", "")) };
    } catch { data[symbol] = { price: null, change: null }; }
  }
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === "/api/market") { await sendMarketData(res, (url.searchParams.get("symbols") || "JPM,BAC,GS,C,SPY").split(",")); return; }
